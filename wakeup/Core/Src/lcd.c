...
void LCD_Set_BWTR(void)
{
	...
	if(lcddev.id==0x9341)
	{
		...
		FSMC_Bank1E->BWTR[0]|=3<<8; 	//数据保存时间(DATAST)为6ns*3个HCLK=18ns
	}
	else if((lcddev.id==0x9486)||(lcddev.id==0x9488)||(lcddev.id==0x5310)||(lcddev.id==0x7796)||(lcddev.id==0x1963))
	{
		FSMC_Bank1E->BWTR[0]|=4<<0;		//地址建立时间(ADDSET)为4个HCLK =24ns  	 
...
//************* ST7796S初始化**********//	
		...
		//************* R61509V或7793初始化**********//	
			...
			LCD_WriteReg(0x0401, 0x0001);	 //R401H Bit0 REV位是数据取反设置
...
//************* SSD1963初始化**********//	
			...
			LCD_PWM_BackLightSet(100); //PWM设置背光为最亮
			lcddev.LCD_W=480;
			lcddev.LCD_H=800;
			break;
		}
...
{ 
	...
	if((lcddev.id==0x9341)||(lcddev.id==0x9486)||(lcddev.id==0x9488)||(lcddev.id==0x5310)||(lcddev.id==0x7796))
	{
		lcddev.setxcmd=0x2A;
		lcddev.setycmd=0x2B;
		lcddev.wramcmd=0x2C;
...
u16 LCD_Read_ID(void)
{
  ...
  LCD_ReadReg(0xC500,&val[1],1);//读回0X55
	LCD_ReadReg(0xC501,&val[2],1);//读回0X10	
	val[0]=(val[1]<<8)|val[2];
	if(val[0]==0x5510)
	{
		return 0x5510;
