...
const u8 send_test[] = "Hello Phung Minh Duc"; //д������

/*****************************************************************************
 * @name       :void SPI_W25Q64_test(void)
 * @date       :2018-08-09 
 * @function   :show SPI test
 * @parameters :None
 * @retvalue   :None
******************************************************************************/
void SPI_W25Q64_test(void)
{
	USART1_Send("spi w25q64 test\r\n");
	printf("spi w25q64 test\r\n");
	Gui_StrCenter(0, 20, RED, BLUE, "STM32F407VxT6��Сϵͳ������", 16, 1); //������ʾ
	Gui_StrCenter(0, 45, RED, BLUE, "SPI ���Գ���", 16, 1);				//������ʾ
	Gui_StrCenter(0, 70, BLUE, BLUE, "KEY_UP:Write  KEY0:Read", 16, 1); //������ʾ
	while (W25QXX_ReadID() != W25Q64)
	{
		Gui_StrCenter(0, 95, RED, WHITE, "W25Q64 Check Failed!", 16, 0);
		delay_ms(500);
		Gui_StrCenter(0, 95, RED, WHITE, "Please Check!       ", 16, 0);
		delay_ms(500);
		LED0 = !LED0; //LED0��˸
	}
	Gui_StrCenter(0, 95, BLUE, WHITE, "W25Q64 Ready!", 16, 0);
	Gui_StrCenter(0, 165, BLUE, BLUE, "2020-06-05", 16, 1);				 //������ʾ
	Gui_StrCenter(0, 190, GREEN, BLUE, "http://www.lcdwiki.com", 16, 1); //������ʾ
}

/*****************************************************************************
 * @name       :void Show_W25Q64_Write(void)
 * @date       :2018-08-09 
 * @function   :show W25Q64 write
 * @parameters :None
 * @retvalue   :None
******************************************************************************/
void Show_W25Q64_Write(void)
{
	LCD_Fill(0, 140, lcddev.width - 1, 156, WHITE);
	Show_Str(lcddev.width / 2 - 84, 120, RED, WHITE, "Start Write W25Q64....", 16, 0);
	W25Q64_Write((u8 *)send_test, FLASH_SIZE - 64, sizeof(send_test));
	Show_Str(lcddev.width / 2 - 84, 120, RED, WHITE, "W25Q64 Write Finished!", 16, 0); //��ʾ�������
}

/*****************************************************************************
 * @name       :void Show_W25Q64_Read(void)
 * @date       :2018-08-09 
 * @function   :show W25Q64 read
 * @parameters :None
 * @retvalue   :None
******************************************************************************/
void Show_W25Q64_Read(void)
{
	u8 rece_buf[sizeof(send_test)];
	Show_Str(lcddev.width / 2 - 84, 120, RED, WHITE, "Start Read W25Q64.... ", 16, 0);
	W25Q64_Read(rece_buf, FLASH_SIZE - 64, sizeof(send_test));
	Show_Str(lcddev.width / 2 - 84, 120, RED, WHITE, "The Data Readed Is:   ", 16, 0);
	Show_Str(lcddev.width / 2 - 84, 140, BLUE, WHITE, rece_buf, 16, 0); //��ʾ�������ַ���
}
