...
{
	/*
	u16 i=0,j=0,num=0,len;
	num=SEND_SIZE;
	while(num)
	{
		if(num>(sizeof(send_test)+2))
		{
			num-=(sizeof(send_test)+2);
			len=(sizeof(send_test)+2);
			j++;
		}
		else
		{
		...
		for(i=0;i<len;i++)
		{
			if(i<(len-2))
			{
				sbuf[j*(sizeof(send_test)+2)+i]=send_test[i];
			}
			else
			{
				sbuf[j*(sizeof(send_test)+2)+len-2]=0x0d;
				sbuf[j*(sizeof(send_test)+2)+len-1]=0x0a;
			}
...
/*****************************************************************************
 * @name       :void Show_DMA_Trans(void)
 * @date       :2018-08-09 
 * @function   :show DMA Transimit
 * @parameters :None
 * @retvalue   :None
******************************************************************************/
void Show_DMA_Trans(void)
{
	float pro = 0; //����
