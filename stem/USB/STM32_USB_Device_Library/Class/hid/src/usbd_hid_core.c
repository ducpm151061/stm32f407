...
uint8_t checkCommand(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (strncmp((char *)rxBuff, command, 28) == 0)
  {
...
void sendFlashData(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  DebugPrint("dataSend %s\r\n", data_send);
  DebugPrint("rxBuff: %s\r\n", (char *)(rxBuff));
  DebugPrint("record index: %lu\r\n", GetIndexStore());
  DebugPrint("offset: %d\r\n", offset);
  DebugPrint("take: %d\r\n", take);
  if (index_view == 0)
    return;
  ...
  /* DebugPrint("index view: %d\r\n", index_view);
  for (i = 0; i <= GetIndexStore(); i++)
  {
    if (index_view >= i)
    {
      memset(data_send, 0, sizeof(data_send));
      Record_Read(index_view - i);
      sprintf(data_send, "%d,%s,%s,%s,%d:%d:%d-%d/%d/%d,%s,%.4f,%.4f,%s,%s,%lf\n", xSystem.Record.Name.IndexStore, (char *)xSystem.Record.Name.NguoiThucHien, (char *)xSystem.Record.Name.DonViCongTac, (char *)xSystem.Record.Name.NguoiLamChung, xSystem.Record.Name.Time.Hour, xSystem.Record.Name.Time.Minute, xSystem.Record.Name.Time.Second, xSystem.Record.Name.Time.Day, xSystem.Record.Name.Time.Month, xSystem.Record.Name.Time.Year, (char *)xSystem.Record.Name.DiaDiemKiemTra, xSystem.Record.Name.KinhDo.value, xSystem.Record.Name.ViDo.value, (char *)xSystem.Record.Name.TenLaiXe, (char *)xSystem.Record.Name.BienSo, xSystem.Record.Name.KetQua.value);
  ...
  if (offset + take <= index_view)
  {
    for (j = offset; j < offset + take; j++)
    {
      ...
      snprintf(data_send, sizeof(data_send), "%d,%s,%s,%s,%d:%d:%d-%d/%d/%d,%s,%.4f,%.4f,%s,%s,%lf\n", xSystem.Record.Name.IndexStore, (char *)xSystem.Record.Name.NguoiThucHien, (char *)xSystem.Record.Name.DonViCongTac, (char *)xSystem.Record.Name.NguoiLamChung, xSystem.Record.Name.Time.Hour, xSystem.Record.Name.Time.Minute, xSystem.Record.Name.Time.Second, xSystem.Record.Name.Time.Day, xSystem.Record.Name.Time.Month, xSystem.Record.Name.Time.Year, (char *)xSystem.Record.Name.DiaDiemKiemTra, xSystem.Record.Name.KinhDo.value, xSystem.Record.Name.ViDo.value, (char *)xSystem.Record.Name.TenLaiXe, (char *)xSystem.Record.Name.BienSo, xSystem.Record.Name.KetQua.value);
  ...
  // Flush bo dem RX
  rxBuff[0] = 0x00;
  DebugPrint("\rSend data successful!!!!!\r\n");
  // DCD_EP_Flush(pdev, 0x81);
}
/**
* @}
