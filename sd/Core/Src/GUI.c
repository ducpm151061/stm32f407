 ...
 * @name       :void LCD_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u16 color)
 * @date       :2018-08-09 
 * @function   :fill the specified area useing color block
 * @parameters :sx:the bebinning x coordinate of the specified area
                sy:the bebinning y coordinate of the specified area
								ex:the ending x coordinate of the specified area
								ey:the ending y coordinate of the specified area
								color:the filled color value point
 * @retvalue   :None
********************************************************************/
...
{  	
	...
	for(i=0;i<height;i++)
	{
		for(j=0;j<width;j++)
		{
			...
			Lcd_WriteData_16Bit(colors[i*width+j]);	//写入数据 	 
		}
	}
}
...
void Gui_StrCenter(u16 x, u16 y, u16 fc, u16 bc, u8 *str,u8 size,u8 mode)
{
	...
	POINT_COLOR=fc;
	//BACK_COLOR=bc;
	Show_Str(x1,y,lcddev.width,size,str,size,mode);
...
void Draw_Window(uint16_t xstart,uint16_t ystart,uint16_t xend,uint16_t yend,uint8_t* caption)
{
	...
	POINT_COLOR = WHITE;
	BACK_COLOR = DARKBLUE;
	Show_Str(xstart+5, ystart+6,lcddev.width,16,caption,16,1);
