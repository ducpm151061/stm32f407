...
u8 updata_fontx(u16 x,u16 y,u8 size,u8 *fxpath,u8 fx)
{
 	...
 	if(rval==0)	 
	{
		...
		while(res==FR_OK)//死循环执行
		{
			...
			W25Q64_Write(tempbuf,offx+flashaddr,4096);		//从0开始写入4096个数据  
	  		offx+=bread;	  
...
{	
	...
	if(rval==0)//字库文件都存在.
	{  
		LCD_ShowString(x,y,size,"Erasing sectors... ",0);//提示正在擦除扇区	
		for(i=0;i<FONTSECSIZE;i++)	//先擦除字库区域,提高写入速度
		{
			fupd_prog(x+20*size/2,y,size,FONTSECSIZE,i);//进度显示
			W25Q64_Read((u8*)buf,((FONTINFOADDR/4096)+i)*4096,4096);//读出整个扇区的内容
			for(j=0;j<1024;j++)//校验数据
			{
				if(buf[j]!=0XFFFFFFFF)break;//需要擦除  	  
			}
			if(j!=1024)W25Q64_Erase_Sector((FONTINFOADDR/4096)+i);	//需要擦除的扇区
		}
		myfree(SRAMIN,buf);
		...
		//全部更新好了
		ftinfo.fontok=0XAA;
		W25Q64_Write((u8*)&ftinfo,FONTINFOADDR,sizeof(ftinfo));	//保存字库信息
	}
