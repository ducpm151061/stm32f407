...
int main(void)
{
	...
	// IIC_24C02_test();								//IIC test
													//ѭ������
	USART1_printf(USART1,"init lcd i2c\r\n");
	I2C_LCD_Init();
	USART1_printf(USART1,"finish");
	u32 count=0;
	while (1)
	{
		count++;
		char temp[50];
		sprintf(temp,"count: %lu",count);
		I2C_LCD_Clear();
		I2C_LCD_BackLight(1);
		I2C_LCD_Puts("Phung Minh Duc");
		I2C_LCD_NewLine();
		I2C_LCD_Puts((char*)temp);
		delay_ms(1000);
		key_value = KEY_Scan(0);
		if (key_value == KEY_UP_PRES) //KEY_UP����
		{
			Show_24C02_Write(); //д��24C02
		}
