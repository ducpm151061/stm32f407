...
static u8 u8LCD_Buff[8]; //bo nho dem luu lai toan bo
static u8 u8LcdTmp;
...
static void I2C_LCD_Write_4bit(u8 u8Data);
static void I2C_LCD_FlushVal(void);
static void I2C_LCD_WriteCmd(u8 u8Cmd);
...
void I2C_LCD_FlushVal(void)
{
	...
	LCD_IIC_Start();
	LCD_IIC_Send_Byte(I2C_LCD_ADDR);
	LCD_IIC_Wait_Ack();
	LCD_IIC_Send_Byte(u8LcdTmp);
	LCD_IIC_Wait_Ack();
...
void I2C_LCD_Write_4bit(u8 u8Data)
{
...
void LCD_WaitBusy(void)
{
	...
	do
	{
		...
		for (i = 0; i < 1; ++i)
		{
			pData[i] = LCD_IIC_Read_Byte(1);
		}
		pData[i] = LCD_IIC_Read_Byte(0);
...
void I2C_LCD_BackLight(u8 u8BackLight)
{
