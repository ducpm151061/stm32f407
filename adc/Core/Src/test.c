...
void ADC_test(void)
{
	printf("adc test\r\n");
	Gui_StrCenter(0,40,RED,BLUE,"STM32F407VxT6最小系统开发板",16,1);//居中显示
	Gui_StrCenter(0,65,RED,BLUE,"ADC 测试程序",16,1);//居中显示	
	Show_Str(lcddev.width/2-68,90,BLUE,BLUE,"ADC1_CH5_VAL:",16,1);//居中显示	
	Show_Str(lcddev.width/2-68,115,BLUE,BLUE,"ADC1_CH5_VOL:",16,1);//居中显示
	Show_Str(lcddev.width/2+36,115,RED,BLUE,"0.000V",16,1);//居中显示
...
void Show_ADC_Value(void)
{
	...
	LCD_ShowNum(lcddev.width/2+36,90,adcx,4,16,0);    //显示ADCC采样后的原始值
	temp=(float)adcx*(3.3/4096);          //获取计算后的带小数的实际电压值，比如3.1111
	adcx=temp;                            //赋值整数部分给adcx变量，因为adcx为u16整形
	LCD_ShowNum(lcddev.width/2+36,115,adcx,1,16,0);    //显示电压值的整数部分，3.1111的话，这里就是显示3
	temp-=adcx;                           //把已经显示的整数部分去掉，留下小数部分，比如3.1111-3=0.1111
	temp*=1000;                           //小数部分乘以1000，例如：0.1111就转换为111.1，相当于保留三位小数。
	LCD_ShowNum(lcddev.width/2+52,115,temp,3,16,1); //显示小数部分（前面转换为了整形显示），这里显示的就是111.
}
