...
/*****************************************************************************
 * @name       :void DAC_test(void)
 * @date       :2018-08-09 
 * @function   :show ADC test
 * @parameters :None
 * @retvalue   :None
******************************************************************************/
void DAC_test(void)
{
	...
	Gui_StrCenter(0,45,RED,BLUE,"DAC 测试程序",16,1);//居中显示	
	Gui_StrCenter(0,70,BLUE,BLUE,"WK_UP:+  KEY0:-",16,1);//居中显示	
	Show_Str(lcddev.width/2-52,95,BLUE,BLUE,"DAC1_VAL:",16,1);//居中显示
	Show_Str(lcddev.width/2-52,120,BLUE,BLUE,"DAC1_VOL:",16,1);//居中显示
	Show_Str(lcddev.width/2+20,120,RED,BLUE,"0.000V",16,1);//居中显示	
	Show_Str(lcddev.width/2-52,145,BLUE,BLUE,"ADC1_VOL:",16,1);//居中显示
	Show_Str(lcddev.width/2+20,145,RED,BLUE,"0.000V",16,1);//居中显示
	Gui_StrCenter(0,170,BLUE,BLUE,"2020-06-05",16,1);//居中显示
	Gui_StrCenter(0,195,GREEN,BLUE,"http://www.lcdwiki.com",16,1);//居中显示
}

/*****************************************************************************
 * @name       :void Show_ADC_Value(void)
 * @date       :2018-08-09 
 * @function   :show ADC value
 * @parameters :None
 * @retvalue   :None
******************************************************************************/
void Show_ADC_Value(void)
{
	u16 adcx;
	float temp;
	adcx=Get_ADC_Average_Value(ADC_Channel_5,20);//获取通道5的转换值，20次取平均
	temp=(float)adcx*(3.3/4096);          //获取计算后的带小数的实际电压值，比如3.1111
	adcx=temp;                            //赋值整数部分给adcx变量，因为adcx为u16整形
	LCD_ShowNum(lcddev.width/2+20,145,adcx,1,16,0);    //显示电压值的整数部分，3.1111的话，这里就是显示3
	temp-=adcx;                           //把已经显示的整数部分去掉，留下小数部分，比如3.1111-3=0.1111
	temp*=1000;                           //小数部分乘以1000，例如：0.1111就转换为111.1，相当于保留三位小数。
	LCD_ShowNum(lcddev.width/2+36,145,temp,3,16,1); //显示小数部分（前面转换为了整形显示），这里显示的就是111.
}

/*****************************************************************************
 * @name       :void Show_DAC_Value(void)
 * @date       :2018-08-09 
 * @function   :show DAC value
 * @parameters :None
 * @retvalue   :None
******************************************************************************/
void Show_DAC_Value(void)
{
	...
	LCD_ShowNum(lcddev.width/2+20,95,adcx,4,16,0);     	   //显示DAC寄存器值
	temp=(float)adcx*(3.3/4096);			         //得到DAC电压值
	adcx=temp;
 	LCD_ShowNum(lcddev.width/2+20,120,temp,1,16,0);     	   //显示电压值整数部分
 	temp-=adcx;
	temp*=1000;
	LCD_ShowNum(lcddev.width/2+36,120,temp,3,16,1); 	   //显示电压值的小数部分
}
